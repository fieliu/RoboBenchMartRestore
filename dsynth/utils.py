...
from mani_skill.utils.wrappers.record import RecordEpisode, parse_env_info
from mani_skill.utils.io_utils import dump_json

@dataclass
class StepSuccLength:
    state: np.ndarray
    ...
    success_length: np.ndarray = None
    fail: np.ndarray = None


class RecordEpisodeSuccLength(RecordEpisode):
    ...
    def __init__(
        ...
        self._trajectory_buffer: StepSuccLength = None
    ...
    def reset(
        ...
        **kwargs,
        ...
        obs, info = gym.Wrapper.reset(self, *args, seed=seed, options=options, **kwargs)
        if info["reconfigure"]:
        ...
        if self.save_trajectory:
            ...
            first_step = StepSuccLength(
                ...
                success_length=np.zeros((1, self.num_envs), dtype=float),
            ...
            if self._trajectory_buffer is None:
                ...
                if self._trajectory_buffer.success is not None:
                    ...
                    recursive_replace(
                        self._trajectory_buffer.success_length, first_step.success_length
                    )
    ...
    def step(self, action):
        if self.save_video and self._video_steps == 0:
            ...
            self.render_images.append(self.capture_image())
        obs, rew, terminated, truncated, info = gym.Wrapper.step(self, action)
        ...
        if self.save_trajectory:
            ...
            if "success_length" in info:
                self._trajectory_buffer.success_length = common.append_dict_array(
                    self._trajectory_buffer.success_length,
                    common.to_numpy(common.batch(info["success_length"])),
                )
            else:
                self._trajectory_buffer.success_length = None
            if "fail" in info:
    ...
    def flush_trajectory(
        ...
        for env_idx in env_idxs_to_flush:
            ...
            if save:
                ...
                if self._trajectory_buffer.success_length is not None:
                    group.create_dataset(
                        "success_length",
                        data=self._trajectory_buffer.success_length[
                            start_ptr + 1 : end_ptr, env_idx
                        ],
                        dtype=float,
                    )
                    episode_info.update(
                        success_length=self._trajectory_buffer.success_length[end_ptr - 1, env_idx]
                    )
                if self._trajectory_buffer.fail is not None:
                    group.create_dataset(
                        "fail",
        ...
        if flush_count > 0:
            ...
            if self._trajectory_buffer.success_length is not None:
                self._trajectory_buffer.success_length = common.index_dict_array(
                    self._trajectory_buffer.success_length, slice(min_env_ptr, N)
                )
            if self._trajectory_buffer.fail is not None:
